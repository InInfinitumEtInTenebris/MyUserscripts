...
(function () {
    ...
    const BLOCK_KEY = 'TextReplacer_BLOCK_GLOBAL'; // Version-agnostic key
    ...
    function loadSettings() {
        // Migrate legacy blocklist to version-agnostic key
    ...
    function compileRuleRegex(rule) {
        ...
        const branchRegexes = parts.map(part => {
            ...
            if (part.includes('---')) {
                ...
                parsedPart = subParts.map(sp => escapeRegExp(sp)).join('\\s+(.*?)\\s+');
            } else {
                parsedPart = escapeRegExp(part);
            }
        ...
        return new RegExp(`(${combined})`, rule.caseSensitive ? 'g' : 'gi');
    }
    ...
    function buildFullGUI() {
        ...
        const searchInput = guiBox.querySelector('#tr-search-input');
        
        searchInput.addEventListener('input', (e) => {
            searchQuery = e.target.value;
            listContainer.scrollTop = 0; // Reset scroll when searching
            renderVirtualList();
        });
    ...
    async function addRuleInteractive(prefill = '') {
        ...
        await dbPut(r); localRules = await dbGetAll(); scheduleWriteMasterMirror(); await runDetectionAndApplyInternal(); updateGuiIfNeeded();
    }
    ...
    async function deleteRuleInteractive(id) {
        if (confirm('Delete this rule permanently?')) { await dbDelete(id); localRules = await dbGetAll(); scheduleWriteMasterMirror(); await runDetectionAndApplyInternal(); updateGuiIfNeeded(); }
    }
    ...
    async function importRulesFromJSONString(jsonStr, merge = true) {
        ...
        localRules = await dbGetAll(); scheduleWriteMasterMirror(); await runDetectionAndApplyInternal(); updateGuiIfNeeded(); alert('Import complete.');
    }
    ...
    function showSettings() {
        ...
        guiBox.innerHTML = `
            <div class="mui-header" style="justify-content: flex-start; gap: 12px;">
                <button class="mui-icon-btn" id="tr-back-btn">⬅</button>
                <h2>Settings</h2>
            </div>
            <div class="mui-list-container" id="tr-set-cont"></div>
        `;
    ...
    function applyStyles() {
        ...
        style.textContent = `
            ...
            .mui-bottom-actions { margin-top:16px; display:flex; flex-direction:column; gap:12px; flex-shrink: 0;}
