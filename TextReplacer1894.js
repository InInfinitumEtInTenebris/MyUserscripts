// ==UserScript==
// @name         Text Replacer — Stable UI + Persistent Scroll + Blocklist Import + Reliable "Sync to this site"
// @namespace    http://tampermonkey.net/
// @version      9.2.3
// @description  Stable GUI, persistent scroll, blocklist import/export, MASTER/ACTIVE/IDB pipeline. Fixed reliable "Sync to this site" that writes MASTER + ACTIVE and emits a SYNC tick.
// @match        *://*/*
// @grant        GM_getValue
// @grant        GM_setValue
...
(function () {
  ...
  const SYNC_KEY = 'TextReplacer_SYNC_v2'; // new tick key to force listeners
  ...
  let blockedDomains = [];
  let guiBox = null, listContainer = null, fab = null;
  let isGuiOpen = false;
  ...
  function signatureOf(r) { return `${r.oldText}:::${r.newText}:::${!!r.caseSensitive}:::${!!r.forceGlobal}`; }
  ...
  function writeGM(key, value) {
    try { GM_setValue(key, JSON.stringify(value)); } catch (e) { console.warn('writeGM failed', e); }
  }
  ...
  // ---------- MASTER mirror (debounced & immediate writer) ----------
  ...
  async function writeMasterImmediately() {
    try {
      writingMaster = true;
      const rules = await dbGetAll();
      writeGM(MASTER_KEY, { ts: now(), rules: rules.map(r => ({ ...r })) });
  ...
  function performReplacementPass() {
    ...
    } catch (e) { console.warn('performReplacementPass error', e); }
  }
  ...
  function scheduleGuiUpdate() {
    ...
    guiUpdateTimer = setTimeout(() => { guiUpdateTimer = null; renderOrPatchGUI(); }, GUI_UPDATE_DEBOUNCE);
  }
  ...
  function renderOrPatchGUI() {
    ...
    for (const r of (localRules || []).sort((a, b) => (a.oldText || '').localeCompare(b.oldText || ''))) {
      ...
      if (!el) {
        ...
        if (metaNode) metaNode.textContent = `${isActive ? '✅ Active' : '💤 Idle (Stored)'} | ${r.forceGlobal ? '🌍 Global' : '🤖 Auto-Detect'}`;
  ...
  function buildFullGUI() {
    ...
    if (typeof saved === 'number') {
      setTimeout(() => { if (listContainer) listContainer.scrollTop = saved; }, 0);
    }

    listContainer.addEventListener('pointerdown', () => {
  ...
  function makeRuleCardElement(r) {
    ...
    item.setAttribute('data-old', r.oldText || '');
  ...
  // ---------- Save / Load UI scroll ----------
  function saveUIScrollForHost(host, offset) {
    try {
      const map = readGM(UI_SCROLL_KEY, {}) || {};
      map[host] = offset;
      writeGM(UI_SCROLL_KEY, map);
    } catch (e) { console.warn('saveUIScrollForHost failed', e); }
  }
  function loadUIScrollForHost(host) {
    try {
      const map = readGM(UI_SCROLL_KEY, {}) || {};
      const v = map[host];
  ...
  function saveUIScrollDebounced() {
    ...
    scrollSaveTimer = setTimeout(() => {
      saveUIScrollForHost(HOST, offset);
  ...
  async function addRuleInteractive() {
    ...
    const t = now();
    const r = { id: uuid(), oldText: oldText.trim(), newText: newText.trim(), caseSensitive, forceGlobal, enabled: true, createdAt: t, updatedAt: t, site: HOST };
    await dbPut(r);
    localRules = await dbGetAll();
  ...
  async function exportRulesFile() {
    ...
    const a = document.createElement('a'); a.href = url; a.download = `text-replacer-rules-${now()}.json`; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
  ...
  function promptFileImport(merge = true) {
    ...
    input.onchange = async (e) => {
      ...
      } catch (err) { alert('Failed to import file: ' + err); } finally { input.remove(); }
    };
  ...
  async function importRulesFromJSONString(jsonStr, merge = true) {
    ...
    for (const r of rulesArr) {
      if (!r || !r.oldText) continue;
      const cand = { id: r.id || uuid(), oldText: r.oldText, newText: r.newText || '', caseSensitive: !!r.caseSensitive, forceGlobal: !!r.forceGlobal, enabled: r.enabled !== false, createdAt: r.createdAt || now(), updatedAt: r.updatedAt || now(), site: r.site || null };
      if (byId.has(cand.id)) {
        const local = byId.get(cand.id);
        if ((cand.updatedAt || 0) > (local.updatedAt || 0)) await dbPut({ ...local, ...cand, id: local.id });
      } else {
  ...
  function promptBlocklistImport() {
    ...
    input.onchange = async (e) => {
      ...
      try {
        const text = await f.text();
        await importBlocklistFromJSONString(text);
      } catch (err) { alert('Failed to import blocklist file: ' + err); } finally { input.remove(); }
    };
    document.body.appendChild(input); input.click();
  }
  async function importBlocklistFromJSONString(jsonStr) {
    let payload;
    try { payload = JSON.parse(jsonStr); } catch (e) { alert('Invalid JSON for blocklist'); return; }
    let arr = null;
    if (Array.isArray(payload)) arr = payload;
    else if (Array.isArray(payload.blocklist)) arr = payload.blocklist;
    else if (Array.isArray(payload.blocked)) arr = payload.blocked;
    else if (Array.isArray(payload.domains)) arr = payload.domains;
    ...
    if (!allStrings) { alert('Blocklist entries must be strings (hostnames).'); return; }
    if (!confirm(`Importing will replace your current blocklist with ${arr.length} entries. Proceed?`)) return;
    blockedDomains = arr;
    saveBlocked();
    showSettings();
    alert('Blocklist imported successfully.');
  }

  // ---------- GUI & settings (includes fixed back button + import/export + Sync) ----------
  ...
  function createGUI() {
    ...
    const toggle = document.createElement('div'); toggle.className = 'mui-toggle'; toggle.textContent = '☰'; toggle.onclick = () => { isGuiOpen = !isGuiOpen; guiBox.classList.toggle('mui-hidden', !isGuiOpen); fab.classList.toggle('mui-hidden', !isGuiOpen); if (isGuiOpen) renderOrPatchGUI(); };
    document.documentElement.append(guiBox, fab, toggle);
  }

  function displayRules() {
    if (!guiBox) return;
    if (!listContainer) buildFullGUI();
    else renderOrPatchGUI();
  }
  ...
  function showSettings() {
    ...
    const blTitle = document.createElement('div'); blTitle.textContent = 'Blocklist'; blTitle.style.fontWeight = '600'; container.appendChild(blTitle);
    ...
    // Export Blocklist
    const exportBlockBtn = document.createElement('button');
    exportBlockBtn.textContent = 'Export Blocklist';
    exportBlockBtn.className = 'mui-button full-width';
    exportBlockBtn.style.marginTop = '8px';
    exportBlockBtn.onclick = async () => {
      try {
        const json = JSON.stringify(blockedDomains || [], null, 2);
        try {
          await navigator.clipboard.writeText(json);
          const blob = new Blob([json], { type: 'application/json' });
          const url = URL.createObjectURL(blob);
          const a = document.createElement('a'); a.href = url; a.download = `text-replacer-blocklist-${now()}.json`; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
          alert('Blocklist copied to clipboard and saved to downloads.');
        } catch (clipErr) {
          const blob = new Blob([json], { type: 'application/json' });
          const url = URL.createObjectURL(blob);
          const a = document.createElement('a'); a.href = url; a.download = `text-replacer-blocklist-${now()}.json`; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
          alert('Blocklist exported to downloads.');
        }
      } catch (e) {
        alert('Failed to export blocklist: ' + e);
      }
    };
    container.appendChild(exportBlockBtn);

    // Import Blocklist
    const importBlockBtn = document.createElement('button');
    importBlockBtn.textContent = 'Import Blocklist';
    importBlockBtn.className = 'mui-button full-width';
    importBlockBtn.style.marginTop = '8px';
    importBlockBtn.onclick = () => { promptBlocklistImport(); };
    container.appendChild(importBlockBtn);

    // Manual sync for this site (RELIABLE)
    const syncTitle = document.createElement('div'); syncTitle.textContent = 'Sync'; syncTitle.style.fontWeight = '600'; syncTitle.style.marginTop = '12px';
    container.appendChild(syncTitle);
    ...
    syncBtn.onclick = async () => {
      try {
        await forceSyncToThisSite(); // reliable atomic sync
        alert('Sync to this site completed.');
      } catch (e) { alert('Sync failed: ' + e); }
    };
    syncRow.append(syncBtn);
    container.append(syncRow);

    guiBox.appendChild(container);

    const backBtn = document.createElement('button');
    backBtn.textContent = '⬅ Back';
    backBtn.className = 'mui-button full-width';
  ...
  async function forceSyncToThisSite() {
    // 1) ensure localRules up-to-date
    localRules = await dbGetAll();

    // 2) write MASTER mirror immediately (full library)
    writeGM(MASTER_KEY, { ts: now(), rules: localRules.map(r => ({ ...r })) });
    lastMasterPayloadTs = now();

    // small pause to let other runtimes observe MASTER (and to avoid races)
    await new Promise(res => setTimeout(res, 120));

    // 3) compute detected rules for current page (use same detection logic)
    const bodyText = document.body ? (document.body.innerText || document.body.textContent || '') : '';
    const detected = [];
    for (const r of localRules) {
      if (!r.enabled) continue;
      if (testRuleAgainstText(r, bodyText)) detected.push(r);
    }

    // 4) write ACTIVE host entry for this host (so other tabs/sites see it)
    updateActiveHostInGM(detected);

    // 5) emit a SYNC tick to force listeners that rely on value-change events
    try {
      writeGM(SYNC_KEY, { ts: now(), host: HOST });
    } catch (e) {
      // ignore
    }

    // 6) re-run local detection and replacement and update GUI
    await new Promise(res => setTimeout(res, 80));
    await runDetectionAndApplyInternal();
    scheduleGuiUpdate();
  }

  // ---------- GM listeners + SYNC support + polling fallback ----------
  try {
    if (typeof GM_addValueChangeListener === 'function') {
      ...
      // SYNC tick listener (optional trigger)
      GM_addValueChangeListener(SYNC_KEY, (name, oldVal, newVal) => {
        try {
          // if sync tick is for this host or global, re-check MASTER/ACTIVE
          const payload = newVal ? (typeof newVal === 'string' ? JSON.parse(newVal) : newVal) : readGM(SYNC_KEY, null);
          if (!payload) return;
          // force merge and active read (fast fn)
          const master = readGM(MASTER_KEY, null);
          if (master && master.ts) mergeMasterPayload(master);
          const active = readGM(ACTIVE_KEY, null);
          if (active && active.ts) handleActiveGMChange(active);
  ...
  // ---------- small utility functions already used above (exposed if needed) ----------
  // runDetectionAndApplyInternal(), mergeMasterPayload(), scheduleWriteMasterMirror(), writeMasterImmediately()
