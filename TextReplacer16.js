// ==UserScript==
// @name         Text Replacer11ZInd (Pro: Clone & Priority)
// @namespace    http://tampermonkey.net/
// @version      5.0.0-CloneUpdate
// @description  Advanced text replacement. Supports "Cloning" Global rules to Local, Local rule priority, and full Database Export. Auto-migrates old data.
// @match        *://*/*
// @grant        none
// @run-at       document-end
// ==/UserScript==
...
(function() {
    ...
    // "replacements" holds the active rules for the current page
    // If conflict (Global vs Local), Local wins.
    let replacements = {}; 
    let blockedDomains = [];
    let guiBox, fab;
    let isGuiOpen = false;
    ...
    const DB_VERSION = 3; // Bumped for Schema Change
    ...
    function initDB() {
        return new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = function(e) {
                ...
                // We need to change keyPath from "oldText" to "id" to allow duplicates across sites
                if (db.objectStoreNames.contains(STORE_RULES)) {
                    const store = transaction.objectStore(STORE_RULES);
                    // Check if we are using the old schema (keyPath = "oldText")
                    if (store.keyPath === "oldText") {
                        console.log("Migrating Database to V3...");
                        // 1. Read all old data
                        store.getAll().onsuccess = function(event) {
                            const oldRules = event.target.result;
                            // 2. Delete old store
                            db.deleteObjectStore(STORE_RULES);
                            // 3. Create new store with 'id'
                            const newStore = db.createObjectStore(STORE_RULES, { keyPath: "id" });
                            // 4. Write data back with generated IDs
                            oldRules.forEach(rule => {
                                const siteKey = rule.isGlobal ? 'GLOBAL' : rule.site;
                                rule.id = `${siteKey}::${rule.oldText}`;
                                newStore.put(rule);
                            });
                        };
                    }
    ...
    function generateId(site, isGlobal, oldText) {
        // Unique ID format: "SITE::TEXT" or "GLOBAL::TEXT"
        const prefix = isGlobal ? 'GLOBAL' : site;
        return `${prefix}::${oldText}`;
    }
    ...
    function loadDataFromDB() {
        return new Promise((resolve, reject) => {
            ...
            // 1. Load Rules (Selective & Priority Logic)
            ...
            ruleStore.getAll().onsuccess = function(e) {
                ...
                // Second pass: Load Local Rules (Overwrite Global if conflict)
                result.forEach(rule => {
    ...
    function saveRuleToDB(rule) {
        return new Promise((resolve, reject) => {
            const transaction = db.transaction([STORE_RULES], "readwrite");
            const store = transaction.objectStore(STORE_RULES);
            // Ensure ID is correct
            rule.id = generateId(rule.site, rule.isGlobal, rule.oldText);
    ...
    function replaceText(node) {
        ...
        if (node.nodeType === Node.TEXT_NODE) {
            ...
            // 'replacements' contains the active mix of Global and Local (Local prioritized)
    ...
    function displayRules() {
        ...
        for (const oldText of keys) {
            ...
            // Visual indicator if this Local rule is overriding a Global one?
            // (Complex to detect cheaply, skipping for now, but implicit by existence)
            ...
            infoDiv.innerHTML = `
                ...
                <div style="font-size:0.8em; opacity:0.8; margin-top:4px;">
                    ...
                    ${caseSensitive ? "Aa Case-Match" : "aa Any-Case"}
                </div>
            `;
    ...
    function showSettings() {
        ...
        // 1. CLONE GLOBAL TO LOCAL (New Feature)
        const cloneDesc = document.createElement('p');
        cloneDesc.textContent = "Create local copies of all Global rules. Useful if you want to modify rules for this site without affecting others.";
        cloneDesc.style.fontSize = "0.85em";
        scrollContainer.appendChild(cloneDesc);

        const cloneBtn = document.createElement('button');
        cloneBtn.textContent = '📋 Clone Global Rules to Local';
        cloneBtn.classList.add('mui-button', 'full-width');
        cloneBtn.style.marginBottom = "10px";
        cloneBtn.onclick = async () => {
            if(confirm(`Copy all Global rules to be Local for "${window.location.hostname}"?`)) {
                const allRules = await getAllRulesFromDB();
                const globalRules = allRules.filter(r => r.isGlobal);
                
                const newRules = globalRules.map(r => ({
                    ...r,
                    isGlobal: false,
                    site: window.location.hostname,
                    // ID will be regenerated by save function
                }));

                await bulkSaveRulesToDB(newRules);
                await loadDataFromDB();
                alert(`Cloned ${newRules.length} rules to Local.`);
                showSettings();
            }
        };
        scrollContainer.appendChild(cloneBtn);

        // 2. MAKE ALL GLOBAL
        const makeGlobalBtn = document.createElement('button');
        makeGlobalBtn.textContent = '🌍 Make Entire Library Global';
        makeGlobalBtn.classList.add('mui-button', 'full-width');
        makeGlobalBtn.style.marginBottom = "10px";
        makeGlobalBtn.onclick = async () => {
            if(confirm("Make EVERY rule in the database Global?")) {
                const allRules = await getAllRulesFromDB();
    ...
    async function addRule() {
        ...
        const isGlobal = confirm("Apply GLOBALLY? (Cancel = Local)");
        ...
        await saveRuleToDB(rule);
        await loadDataFromDB(); // Reload to handle priority logic
        displayRules();
        runReplacements();
    }

    async function editRule(oldText) {
        // Find the rule active in the replacement map
        const rule = replacements[oldText];
        if (!rule) return;
        ...
        // If key (oldText) or scope (Global/Local) changed, delete old entry first
        // Because ID depends on them.
    ...
    async function exportRules() {
        // Explicitly fetch ALL rules from DB regardless of current view
        const allRules = await getAllRulesFromDB();
        ...
        input.addEventListener('change', function(event) {
            ...
            if (file) {
                ...
                reader.onload = async function(e) {
                    ...
                    await bulkSaveRulesToDB(rulesToSave);
                    await loadDataFromDB();
                    alert(`Imported ${count} rules.`);
                    displayRules();
                    runReplacements();
                };
