...
// @version      9.9.5
// @description  Virtualized M3 UI, dynamic theming, full Blocklist/Rule I/O, Multi-Term (|), Gap (---), Filter (#{...}#) operators, Expressive motion, custom CRUD GUI. Smart Priority now always respects length first (longest match wins), then recency/frequency. Fixed blocklist scroll, navigation blanking, added error resilience.
// @match        *://*/*
// @grant        GM_getValue
// @grant        GM_setValue
...
(function () {
    ...
    function performReplacementPass() {
        ...
        // **FIXED SORT**: length always wins – longest term first, Smart only decides tiebreaker for equal length
        let rulesList = Object.values(activeRules).sort((a, b) => {
            const lenDiff = (b.oldText || '').length - (a.oldText || '').length;
            if (lenDiff !== 0) return lenDiff; // longest first regardless of Smart flag

            const aSmart = !!a.smartPriority;
            const bSmart = !!b.smartPriority;
            if (aSmart !== bSmart) return aSmart ? -1 : 1; // Smart before non-Smart when lengths equal

            if (aSmart) { // both Smart: use recency then frequency
