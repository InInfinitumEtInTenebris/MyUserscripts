// ==UserScript==
// @name         Text Replacer — Stable UI + Persistent Scroll + Blocklist Import + Smart Priority
// @namespace    http://tampermonkey.net/
// @version      9.2.5
// @description  Stable GUI, persistent scroll, blocklist import/export, MASTER/ACTIVE/IDB pipeline. Reliable "Sync to this site". Added Smart Priority (Length-based sorting).
// @match        *://*/*
// @grant        GM_getValue
// @grant        GM_setValue
...
(function () {
    ...
    // Updated signature to include smartPriority
    ...
    // ---------- detection & replacement (Sorting Logic Added) ----------
    ...
    function performReplacementPass() {
        try {
            // 1. Convert activeRules map to array
            let rulesList = Object.values(activeRules);

            // 2. SORTING (Smart Recognition Logic)
            // Priority 1: Smart Priority enabled (User selection)
            // Priority 2: Length Descending (Essential for overlapping terms like "United States" vs "United")
            rulesList.sort((a, b) => {
                // If one has smartPriority and the other doesn't, prioritize the one that does
                if (!!a.smartPriority !== !!b.smartPriority) {
                    return a.smartPriority ? -1 : 1;
                }
                // If priority flags are equal, sort by length of text (descending)
                const lenA = (a.oldText || '').length;
                const lenB = (b.oldText || '').length;
    ...
    async function editRuleInteractive(id) {
        ...
        rule.smartPriority = updatedPriority; // save bool
        rule.updatedAt = now();
