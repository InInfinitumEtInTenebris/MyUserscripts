...
// @version      10.2.0
// @description  Virtualized M3 UI, dynamic theming, full Blocklist/Rule I/O, Multi-Term (|), Gap (---), Filter (#{...}#) operators, Cross-Element Replacement (Range API, exact words), Protected Areas (whitelist), dark/light theme, operator cheat sheet, improved settings scroll, Expressive motion, custom CRUD GUI.
// @match        *://*/*
// @grant        GM_getValue
// @grant        GM_setValue
...
(function () {
    ...
    // ---------- PROTECTED AREAS (whitelist for text boxes) ----------
    ...
    function isInProtectedArea(node) {
        if (!protectedSelectors.length) return false;
        const el = node.nodeType === Node.ELEMENT_NODE ? node : node.parentElement;
        if (!el) return false;
        if (el.matches && el.matches(protectedSelectors.join(','))) return true;
        if (el.closest) return !!el.closest(protectedSelectors.join(','));
        return false;
    }
    ...
    function applyCrossMatches(matches) {
        // Sort by start offset descending to apply changes backwards safely
        ...
        for (const m of matches) {
            try {
                ...
                // Use Range API to precisely delete only the matched text
                const range = document.createRange();
                range.setStart(startNode, startOffset);
                range.setEnd(endNode, endOffset);
                range.deleteContents();

                // Insert replacement wrapper at the start point
    ...
    function loadSettings() {
        try {
            ...
            extractThemeColor();
            protectedSelectors = readGM(PROTECTED_KEY, DEFAULT_PROTECTED_SELECTORS) || [];
        } catch (e) { error('loadSettings failed', e); }
    }
    function saveBlocked() { writeGM(BLOCK_KEY, blockedDomains || []); }
    function saveProtectedSelectors() { writeGM(PROTECTED_KEY, protectedSelectors); }
    ...
    function performReplacementPass() {
        ...
        // Pass 0: Cross-element replacement
    ...
    // ---------- DIALOG WITH OPERATOR CHEAT SHEET & DARK THEME ----------
    ...
    // ---------- SETTINGS WITH IMPROVED SCROLLING & PROTECTED AREAS ----------
    function showSettings() {
        ...
        try {
            ...
            protListContainer.style.maxHeight = '200px';  // increased
            protListContainer.style.overflowY = 'auto';
            protListContainer.style.border = '1px solid var(--md-sys-color-surface-container)';
            protListContainer.style.borderRadius = '8px';
            protListContainer.style.padding = '8px';
            protListContainer.style.marginBottom = '12px';

            protectedSelectors.forEach(sel => {
                const row = document.createElement('div'); row.className = 'mui-settings-row';
                row.innerHTML = `<span style="font-weight: 500; word-break: break-all;">${escapeHtml(sel)}</span>`;
                const x = document.createElement('button'); x.textContent = '✖'; x.className = 'mui-icon-btn mui-error';
                x.onclick = () => {
            ...
            blocklistContainer.style.maxHeight = '200px';
            blocklistContainer.style.overflowY = 'auto';
            blocklistContainer.style.border = '1px solid var(--md-sys-color-surface-container)';
            blocklistContainer.style.borderRadius = '8px';
            blocklistContainer.style.padding = '8px';
            blocklistContainer.style.marginBottom = '16px';

            (blockedDomains || []).forEach(d => {
                const row = document.createElement('div'); row.className = 'mui-settings-row';
                row.innerHTML = `<span style="font-weight: 500;">${escapeHtml(d)}</span>`;
                const x = document.createElement('button'); x.textContent = '✖'; x.className = 'mui-icon-btn mui-error';
                x.onclick = () => { blockedDomains = blockedDomains.filter(x => x !== d); saveBlocked(); showSettings(); };
                row.appendChild(x);
            ...
            const blExportBtn = document.createElement('button'); blExportBtn.textContent = 'Export'; blExportBtn.className = 'mui-button mui-pill-settings';
            blExportBtn.onclick = () => {
            ...
            const blImportBtn = document.createElement('button'); blImportBtn.textContent = 'Import'; blImportBtn.className = 'mui-button mui-pill-settings';
            blImportBtn.onclick = () => { promptFileImport(false); };
            blExImpRow.append(blExportBtn, blImportBtn);
            cont.appendChild(blExImpRow);
            ...
            hlBtn.onclick = () => { enableHighlight = !enableHighlight; writeGM(HIGHLIGHT_KEY, enableHighlight); revertAllReplacements(); runDetectionAndApplyInternal(); showSettings(); };
    ...
    function applyStyles() {
        ...
        style.textContent = `
            ...
            .mui-settings-container { display: flex; flex-direction: column; gap: 16px; padding-left: 8px; overflow-y: auto; }
    ...
    async function bootstrap() {
        try {
            ...
            log('Text Replacer initialized successfully with Range API cross-element, dark theme, and operator help.');
        } catch (e) {
