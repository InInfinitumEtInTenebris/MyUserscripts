// ==UserScript==
// @name         Text Replacer — Stable UI + Persistent Scroll + Blocklist Import + Reliable "Sync to this site" (Reinforced)
// @namespace    http://tampermonkey.net/
// @version      9.2.4
// @description  Stable GUI, persistent scroll, blocklist import/export, MASTER/ACTIVE/IDB pipeline. Fixed reliable "Sync to this site" that writes MASTER + ACTIVE and emits a SYNC tick. Reinforced sync logic for high volume.
// @match        *://*/*
// @grant        GM_getValue
// @grant        GM_setValue
...
(function () {
    ...
    // ---------- GM helpers (The Cross-Tab Sync Mechanism) ----------
    ...
    // ---------- MASTER mirror (Pipeline: IDB -> Memory -> GM_Storage) ----------
    ...
    // ---------- intelligent merge (Pipeline: GM_Storage -> IDB -> Memory) ----------
    async function mergeMasterPayload(payload) {
        ...
        try {
            ...
            // 4. Update in-memory state from the newly merged IDB
    ...
    // ---------- ACTIVE host logic (Determines what runs on THIS site) ----------
    ...
    // ---------- detection & replacement (Handles "Infinite" terms via TreeWalker) ----------
    function testRuleAgainstText(rule, text) {
        ...
        // Use word boundaries for Latin tokens to avoid false positives (e.g. replacing "cat" in "concatenate")
        const patt = isLatinToken(old) ? `\\b${escapeRegExp(old)}\\b` : escapeRegExp(old);
        try {
            const re = new RegExp(patt, rule.caseSensitive ? '' : 'i');
            return re.test(text);
        } catch (e) {
            // Fallback for invalid regex characters if escape fails somehow
            return text.indexOf(old) !== -1;
        }
    }

    // 1. Detection Phase: Scans the whole body text once to see which rules *might* apply.
    async function runDetectionAndApplyInternal() {
        if (blockedDomains.includes(HOST)) return;
        localRules = await dbGetAll(); // Ensure memory is synced with IDB before detection
        ...
        // Broadcast results to GM so other tabs know what's active here
    ...
    // 2. Replacement Phase: The heavy lifter using TreeWalker.
    function performReplacementPass() {
        try {
            // TreeWalker is the most efficient way to iterate infinite DOM nodes.
            const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT, null, false);
            let node;
            while ((node = walker.nextNode())) {
                if (!node.parentElement) continue;
                // Skip GUI and technical elements
                ...
                // Cache original text to avoid compounding replacements over multiple passes
                ...
                // Apply all active rules sequentially to this node's text.
                // This allows chained replacements (A->B, then B->C becomes A->C).
                // It does not cause infinite loops within a single node pass.
                ...
                // Only touch the DOM if a change actually occurred
    ...
    function makeRuleCardElement(r) {
        ...
        info.innerHTML = `<div class="mui-rule-text">"${escapeHtml(r.oldText)}" ➡ "${escapeHtml(r.newText)}"</div> <div class="mui-rule-meta">${isActive ? '✅ Active' : '💤 Idle (Stored)'} | ${r.forceGlobal ? '🌍 Global' : '🤖 Auto-Detect'}</div>`;
    ...
    async function editRuleInteractive(id) {
        ...
        rule.oldText = updatedOld.trim(); rule.newText = updatedNew.trim(); rule.caseSensitive = updatedCase; rule.forceGlobal = updatedGlobal; rule.updatedAt = now();
    ...
    function applyStyles() {
        ...
        style.textContent = ` .mui-box { position: fixed; top:50%; left:50%; transform:translate(-50%,-50%); width:90vw; max-width:450px; background:#fff; color:#333; padding:15px; border-radius:12px; box-shadow:0 8px 30px rgba(0,0,0,.45); z-index:2147483647; max-height:85vh; display:flex; flex-direction:column; font-family:Segoe UI, Roboto, sans-serif; font-size:14px;} .mui-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;} .mui-list-container{overflow-y:auto;flex-grow:1;padding-right:4px;} .mui-card{background:#f6f6f6;border-radius:8px;padding:12px;margin:8px 0;display:flex;justify-content:space-between;align-items:center;border:1px solid #eee;} .mui-card-actions{display:flex;gap:8px;} .mui-button,.mui-button-small{background:#6200EE;color:#fff;border:none;border-radius:8px;padding:10px;cursor:pointer;font-weight:500;} .mui-fab{position:fixed;bottom:20px;right:20px;width:56px;height:56px;background:#6200EE;color:white;border-radius:50%;border:none;font-size:28px;z-index:2147483647;display:flex;align-items:center;justify-content:center;} .mui-toggle{position:fixed;top:15vh;left:0;width:40px;height:40px;background:rgba(0,0,0,0.6);border-top-right-radius:8px;border-bottom-right-radius:8px;color:white;text-align:center;line-height:40px;cursor:pointer;z-index:2147483647;font-size:22px;} .mui-hidden{display:none!important;} `;
    ...
    // ---------- reliable force sync implementation (Reinforced Pipeline) ----------
    async function forceSyncToThisSite() {
        // 1) CRITICAL: Force read latest MASTER from GM and merge to IDB first.
        // This ensures IDB is not stale before we use it as the source of truth for this site.
        try {
            const currentMaster = readGM(MASTER_KEY, null);
            if (currentMaster && currentMaster.ts) {
                 await mergeMasterPayload(currentMaster);
            }
        } catch(e) { console.warn("Force sync merge failed", e); }

        // 2) Now ensure local memory is up-to-date with the freshly merged IDB
        localRules = await dbGetAll();

        // 3) Write MASTER mirror immediately (full library back to GM to ensure consistency)
        ...
        // 4) compute detected rules for current page (use same detection logic)
        const bodyText = document.body ? (document.body.innerText || document.body.textContent || '') : '';
        const detected = [];
        for (const r of localRules) {
            if (!r.enabled) continue;
        ...
        // 5) write ACTIVE host entry for this host (so other tabs/sites see it)
        updateActiveHostInGM(detected);

        // 6) emit a SYNC tick to force listeners that rely on value-change events
        try { writeGM(SYNC_KEY, { ts: now(), host: HOST }); } catch (e) { }

        // 7) re-run local detection and replacement and update GUI
    ...
    async function bootstrap() {
        ...
        localRules = await dbGetAll();
        // Initial sync: if IDB is empty, try fetching from GM MASTER
        if ((!localRules || localRules.length === 0)) {
            ...
            // If IDB has data, ensure GM mirror is set
