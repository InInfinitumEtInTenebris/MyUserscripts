...
// @version      9.8.0
// @description  Virtualized M3 UI, dynamic theming, full Blocklist/Rule I/O, Multi-Term (|), Gap (---), Filter (#{...}#) operators, Expressive motion, custom CRUD GUI. Added apostrophe handling and early pattern priming.
// @match        *://*/*
// @grant        GM_getValue
// @grant        GM_setValue
...
(function () {
    ...
    if (earlyRules.length > 0 && !isExcludedContext()) {
        const earlyRxMap = earlyRules.map(r => ({ r, rx: compileRuleRegex(r, false) }));
        const earlyMo = new MutationObserver(mutations => {
